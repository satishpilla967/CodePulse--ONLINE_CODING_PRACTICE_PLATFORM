...
async function main() {
  try {
    ...
    const tags = [
      `tahminator/codepulse:${tagPrefix}latest`,
      `tahminator/codepulse:${tagPrefix}${timestamp}`,
      `tahminator/codepulse:${tagPrefix}${gitSha}`,
    ];
    ...
    try {
      await $`docker buildx create --use --name codepulse-builder`;
    } catch {
      await $`docker buildx use codepulse-builder`;
    }
