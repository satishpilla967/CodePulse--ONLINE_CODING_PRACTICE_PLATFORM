...
async function main() {
  ...
  const tags = [
    `tahminator/codepulse-standup-bot:latest`,
    `tahminator/codepulse-standup-bot:${timestamp}`,
    `tahminator/codepulse-standup-bot:${gitSha}`,
  ];
  ...
  try {
    await $`docker buildx create --use --name codepulse-standup-bot-builder`;
  } catch {
    await $`docker buildx use codepulse-standup-bot-builder`;
  }
