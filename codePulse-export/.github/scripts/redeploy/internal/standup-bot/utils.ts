...
export async function _getOrCreateStandupBotResource({
  ...
  if (
    !apps ||
    !apps.length ||
    !apps.find((app) => app.name === "codepulse-standup-bot")
  ) {
    await client.applications.createDockerImage({
      projectUuid,
      name: "codepulse-standup-bot",
      dockerRegistryImageName: "tahminator/codepulse-standup-bot",
      dockerRegistryImageTag: "latest",
      serverUuid,
      description: "codepulse-standup-bot",
  ...
  const app = apps.find((app) => app.name === "codepulse-standup-bot");

  if (!app) {
    throw new Error("Cannot find codepulse-standup-bot resource");
  }

  if (!app.uuid) {
    throw new Error("codepulse-standup-bot is missing UUID");
  }
