package org.patinanetwork.codepulse.common.db.repos.leaderboard;
...
import org.patinanetwork.codepulse.common.db.repos.leaderboard.options.LeaderboardFilterOptions;
...
import org.patinanetwork.codepulse.common.page.Indexed;
...
public class LeaderboardSqlRepository implements LeaderboardRepository {
    ...
    public boolean disableLeaderboardById(final String leaderboardId) {
        ...
        try (Connection conn = ds.getConnection();
            ...
            stmt.setString("id", leaderboardId);
    ...
    public void addNewLeaderboard(final Leaderboard leaderboard) {
        String sql = """
            INSERT INTO "Leaderboard"
                (id, name, "shouldExpireBy", "syntaxHighlightingLanguage", "createdAt")
            VALUES
                (:id, :name, :shouldExpireBy, :syntaxHighlightingLanguage, :createdAt)
            """;
        leaderboard.setId(UUID.randomUUID().toString());
        leaderboard.setCreatedAt(java.time.LocalDateTime.now());
        try (Connection conn = ds.getConnection();
                NamedPreparedStatement stmt = new NamedPreparedStatement(conn, sql)) {
            stmt.setString("id", leaderboard.getId());
    ...
    public List<Indexed<UserWithScore>> getGlobalRankedIndexedLeaderboardUsersById(
        ...
        String sql = """
                ...
                ORDER BY
                    r.rank ASC
            """
                        .formatted(userIdsInClause(users.size()));

        try (Connection conn = ds.getConnection();
                NamedPreparedStatement stmt = new NamedPreparedStatement(conn, sql)) {
            List<String> userIds =
                    users.stream().map(UserWithScore::getId).collect(Collectors.toList());

            for (int i = 0; i < userIds.size(); i++) {
                stmt.setString("userId" + i, userIds.get(i));
            }
    ...
    /**
     * Builds a comma-separated list of {@code :userId0, :userId1, ...} named placeholders. MySQL has no
     * array type / {@code = ANY(...)} operator, so a dynamically-sized {@code IN (...)} clause is used
     * instead.
     */
    private static String userIdsInClause(final int count) {
        if (count == 0) {
            return "NULL";
        }
    ...
    public List<UserWithScore> getRecentLeaderboardUsers(final LeaderboardFilterOptions options) {
        ...
        String sql = """
            ...
            AND
                (u."discordName" LIKE :searchQuery OR u."leetcodeUsername" LIKE :searchQuery OR u."nickname" LIKE :searchQuery)
            ORDER BY
                m."totalScore" DESC,
                -- The following case is used to put users with linked leetcode names before
                -- those who don't.
    ...
    public List<UserWithScore> getLeaderboardUsersById(final String id, final LeaderboardFilterOptions options) {
        ...
        try (Connection conn = ds.getConnection();
                NamedPreparedStatement stmt = new NamedPreparedStatement(conn, sql)) {
            stmt.setString("leaderboardId", id);
            stmt.setBoolean("patina", options.isPatina());
            stmt.setBoolean("hunter", options.isHunter());
            stmt.setBoolean("nyu", options.isNyu());
            stmt.setBoolean("baruch", options.isBaruch());
    ...
    public boolean addUserToLeaderboard(final String userId, final String leaderboardId) {
        ...
        try (Connection conn = ds.getConnection();
                NamedPreparedStatement stmt = new NamedPreparedStatement(conn, sql)) {
            stmt.setString("id", UUID.randomUUID().toString());
            stmt.setString("userId", userId);
            stmt.setString("leaderboardId", leaderboardId);
    ...
    public int getRecentLeaderboardUserCount(final LeaderboardFilterOptions options) {
        String sql = """
                ...
                AND
                    (u."discordName" LIKE :searchQuery OR u."leetcodeUsername" LIKE :searchQuery)
            """;
        try (Connection conn = ds.getConnection();
                NamedPreparedStatement stmt = new NamedPreparedStatement(conn, sql)) {
            stmt.setBoolean("patina", options.isPatina());
    ...
    public boolean addAllUsersToLeaderboard(final String leaderboardId) {
        ...
        try (Connection conn = ds.getConnection();
            ...
            for (var user : users) {
                ...
                stmt.setString("id", userMetaId);
                stmt.setString("userId", user.getId());
                stmt.setString("leaderboardId", leaderboardId);
                stmt.addBatch();
            }
