...
public class ReactEmailTemplaterImpl implements ReactEmailTemplater {
    ...
    @Override
    public String passwordResetEmailTemplate(final String resetUrl) throws IOException {
        final String html = getHtmlAsString("static/email/password-reset.html");
        final Document doc = Jsoup.parse(html);

        doc.getElementById("input-verifyUrl-innerText").text(resetUrl);
        doc.getElementById("input-verifyUrl-href").attr("href", resetUrl);
