...
/**
 * Update the logged-in user's own nickname and/or profile picture URL.
 */
...
async function updateOwnProfile(body: { nickname?: string; profileUrl?: string }) {
  const { url, method, req, res } = ApiURL.create("/api/user/profile", {
    method: "PATCH",
  });
